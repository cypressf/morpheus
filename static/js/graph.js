...
(function() {
  var Sleep, User, chartC, chartO, currentInteractionState, formatTime, globalChartCOffset, mainUser, position, randint, resizeChart, resizeLines, ticks, updateCurrent, updateOverview;
  ...
  updateCurrent = function() {
    ...
    return chartC.selectAll(".rule").data(the_ticks).enter().append("text").attr("class", "rule").attr("y", function(d) {
      return position(d) * h;
    }).attr("x", 0).attr("dx", 20).attr("text-anchor", "middle").text(function(d) {
      return formatTime(d);
    });
  };
